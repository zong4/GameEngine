...
Engine::OpenGLVertexBuffer::OpenGLVertexBuffer(uint32_t size)
{
    ...
    Logger::EngineTrace(std::format(
        "OpenGL vertex buffer is constructed with ID: {0}", m_RendererID));
}
...
Engine::OpenGLVertexBuffer::~OpenGLVertexBuffer()
{
    ...
    Logger::EngineTrace(std::format(
        "OpenGL vertex buffer is destructed with ID: {0}", m_RendererID));
}
...
void Engine::OpenGLVertexBuffer::SetData(std::span<const float> vertices,
                                         uint32_t offset)
{
    glBindBuffer(GL_ARRAY_BUFFER, m_RendererID);
    glBufferSubData(GL_ARRAY_BUFFER, offset, vertices.size_bytes(),
                    vertices.data());
    ...
    Logger::EngineTrace(std::format(
        "OpenGL vertex buffer is setting data with ID: {0}", m_RendererID));
}

void Engine::OpenGLVertexBuffer::SetData(std::span<const double> vertices,
                                         uint32_t offset)
{
    glBindBuffer(GL_ARRAY_BUFFER, m_RendererID);
    glBufferSubData(GL_ARRAY_BUFFER, offset, vertices.size_bytes(),
