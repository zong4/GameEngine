...
Engine::OpenGLTexture2D::OpenGLTexture2D(uint32_t width, uint32_t height) : Texture2D(width, height)
{
    ...
    Logger::EngineTrace(
        std::format("Texture is constructed with ID: {0}, width: {1}, height: {2}", m_RendererID, m_Width, m_Height));
}
...
void Engine::OpenGLTexture2D::SetData(std::span<const uint32_t> data)
{
    ...
    Logger::EngineTrace(
        std::format("Texture data is set with ID: {0}, width: {1}, height: {2}", m_RendererID, m_Width, m_Height));
}