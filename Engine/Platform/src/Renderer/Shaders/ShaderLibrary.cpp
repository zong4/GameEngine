...
void Engine::ShaderLibrary::Add(const std::string& name,
                                const std::shared_ptr<Shader>& shader)
{
    Logger::EngineAssert(m_Shaders.find(name) == m_Shaders.end(),
                         "Shader already exists in the library");
    m_Shaders[name] = shader;
}

std::shared_ptr<Engine::Shader>
Engine::ShaderLibrary::Load(const std::string& name,
...
std::shared_ptr<Engine::Shader>
Engine::ShaderLibrary::Get(const std::string& name)
{
    Logger::EngineAssert(m_Shaders.find(name) != m_Shaders.end(),
