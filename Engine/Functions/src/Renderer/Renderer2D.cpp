...
static std::unique_ptr<Renderer2DStorage> s_Data =
    std::make_unique<Renderer2DStorage>();
} // namespace Engine
...
void Engine::Renderer2D::Init()
{
    ...
    s_Data->TextureShader = Shader::Create(
        std::string(PROJECT_DIR) + "/Engine/assets/shaders/Texture.glsl");

    constexpr uint32_t width = 2, height = 2;
    s_Data->WhiteTexture = Texture2D::Create(width, height);
    std::uint32_t whiteTextureData[width * height] = {0xffffffff, 0x00ffffff,
...
void Engine::Renderer2D::BeginScene(const OrthographicCamera& camera)
{
    ...
    s_Data->TextureShader->SetUniformMat4f("u_ViewProjection",
...
void Engine::Renderer2D::DrawQuad(const glm::vec2& position, float rotation,
...
void Engine::Renderer2D::DrawQuad(const glm::vec3& position, float rotation,
                                  const glm::vec2& size, const glm::vec4& color,
                                  const std::shared_ptr<Texture2D>& texture,
                                  const glm::vec2& tilingFactor)
{
    s_Data->TextureShader->Bind();
    s_Data->TextureShader->SetUniformMat4f(
        "u_Transform",
